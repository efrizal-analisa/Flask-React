...
with app.app_context():
    db.create_all()
    if db.session.query(User).filter_by(username='efrizal').count() < 1:
        db.session.add(User(
          username='efrizal',
...
def login():
    # app = sdk.App('inT9Ic-BhfqbRA-wgtz8Dn_WHUuAAmSI3VN0kByQpyU').getAppMetaData()
    # email = app.json()['app']['email']
    # print(email)
    req = flask.request.get_json(force=True)
    provider = req.get('provider', None)
    if provider:
        app = sdk.Auth().getAccessToken('efrizal@analisa.io')
